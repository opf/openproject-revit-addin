...
namespace OpenProject.Shared.Math3D
{
  public static class ClippingPlaneUtils
  {
    ...
    /// <summary>
    /// Converts a bcf clipping plane into an axis aligned clipping bounding box. The clipping plane direction vector
    /// (plane normal) is compared to the coordinate axes. If the angle is below a given delta, the plane is converted
    /// to one side of an axis aligned clipping bounding box. The rest of the box's sides are set to max value.
    /// </summary>
    /// <param name="plane">The bcf clipping plane.</param>
    /// <param name="delta">The maximum angle the plane direction can differ from a XYZ coordinate axis.</param>
    /// <returns>The axis aligned bounding box.</returns>
    public static AxisAlignedBoundingBox ToAxisAlignedBoundingBox(this Clipping_plane plane, decimal delta = 0)
    {
